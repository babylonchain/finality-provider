...
func (fp *FinalityProviderInstance) finalitySigSubmissionLoop() {
	...
	for {
		...
		case b := <-fp.poller.GetBlockInfoChan():
			...
			if !should {
				fp.MustSetLastProcessedHeight(nextBlock.Height)
				continue
			}
			...
			if res == nil {
				fp.reportCriticalErr(fmt.Errorf("received an empty response when submitting finality sig"))
				continue
			}
			fp.logger.Info(
		...
		case <-commitRandTicker.C:
			...
			if txRes != nil {
				fp.logger.Info(
					...
					zap.String("pk", fp.GetBtcPkHex()),
					zap.Uint64("last_committed_height", fp.GetLastCommittedHeight()),
...
func (fp *FinalityProviderInstance) tryFastSync(targetBlock *types.BlockInfo) (*FastSyncResult, error) {
	...
	if fp.GetLastCommittedHeight() <= fp.GetLastVotedHeight() {
		if err := fp.SetLastProcessedHeight(targetBlock.Height); err != nil {
			return nil, err
		}
		fp.logger.Debug(
...
// 1. the finality-provider does not have voting power on the given block
// 2. the last committed height is lower than the block height as this indicates the finality-provider
// does not have the corresponding public randomness
// 3. the block height is lower than the last voted height as this indicates that the finality-provider
// does not need to send finality signature over this block
func (fp *FinalityProviderInstance) shouldSubmitFinalitySignature(b *types.BlockInfo) (bool, error) {
	// check last voted height
	if fp.GetLastVotedHeight() >= b.Height {
		fp.logger.Debug(
	...
	if fp.GetLastCommittedHeight() < b.Height {
		fp.logger.Debug(
...
func (fp *FinalityProviderInstance) CommitPubRand(tipBlock *types.BlockInfo) (*types.TxResponse, error) {
	lastCommittedHeight := fp.GetLastCommittedHeight()
...
func (fp *FinalityProviderInstance) TestSubmitFinalitySignatureAndExtractPrivKey(b *types.BlockInfo) (*types.TxResponse, *btcec.PrivateKey, error) {
	...
	if fp.GetLastCommittedHeight() < b.Height {
		return nil, nil, fmt.Errorf("the finality-provider's last committed height %v is lower than the current block height %v",
			fp.GetLastCommittedHeight(), b.Height)
	}
